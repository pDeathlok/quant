...
from quant.research.rule_windows import Z_RULE_HISTORY, rule_tail_start
...
def compute_z_skill_flags(
    df: pd.DataFrame,
    *,
    output_start: str | pd.Timestamp | None = None,
) -> pd.DataFrame:
    # _normalize_daily has already computed recursive features on its complete
    # input. Include raw-flag cooldown context, not just the visible output.
    offset = rule_tail_start(df, output_start, (spec.key for spec in build_signal_specs()), Z_RULE_HISTORY)
    out = df.iloc[offset:].copy()
    pct = out["pct_chg"]
    close = out["close"]
    high = out["high"]
    low = out["low"]
    ...
    minimum_date = max(pd.Timestamp("2020-01-01"), pd.Timestamp(output_start)) if output_start is not None else pd.Timestamp("2020-01-01")
    result = result[result["date"] >= minimum_date].copy()
    result.attrs["finite_rule_rows"] = len(out)
    result.attrs["full_feature_rows"] = len(df)
    return result
...
def process_frame(
    ...
    *,
    ...
    try:
        ...
        if df.empty:
            return None
        signals = compute_z_skill_flags(df, output_start=output_start)
        signal_cols = [spec.key for spec in build_signal_specs()]
        signals = signals[signals[signal_cols].any(axis=1)].copy()
        return signals if not signals.empty else None
    except Exception as exc:
