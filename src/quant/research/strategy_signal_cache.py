...
from quant.research import b1_family_rules as family_rules
from quant.research import z_skill_rules as z_skills
from quant.research.rule_backtest_support import DEFAULT_DAILY_DIR
from quant.research import rule_windows
...
CONTRACT_PATHS = (
    Path(__file__),
    Path(family_rules.__file__),
    Path(z_skills.__file__),
    Path(rule_windows.__file__),
    PACKAGE_ROOT / "data/market_data_store.py",
    PACKAGE_ROOT / "data/source_merge.py",
    PACKAGE_ROOT / "data/factors/__init__.py",
    PACKAGE_ROOT / "data/factors/alpha101.py",
    PACKAGE_ROOT / "data/factors/alpha191.py",
    PACKAGE_ROOT / "data/factors/base.py",
    PACKAGE_ROOT / "data/factors/data_adapter.py",
    PACKAGE_ROOT / "data/factors/momentum.py",
    PACKAGE_ROOT / "data/factors/technical.py",
    PACKAGE_ROOT / "features/b1_gate.py",
    PACKAGE_ROOT / "features/daily_factor_layer.py",
    PACKAGE_ROOT / "features/project_factor_layer.py",
    PACKAGE_ROOT / "features/variable_library.py",
    PACKAGE_ROOT / "strategies/custom/triple_volume_breakout.py",
    PACKAGE_ROOT / "strategies/custom/vegas_tunnel.py",
    ...
    return _stable_json_fingerprint(
        {
            ...
            "rule_window_version": rule_windows.RULE_WINDOW_VERSION,
...
def _partitioned_source_identity(
    ...
    *,
    ...
    pinned = current_market_snapshot()
    if pinned is not None:
        return {
            "fingerprint": _stable_json_fingerprint({
                "dataset": pinned.manifest["datasets"][daily_dir.name],
                "history_start": history_start.strftime("%Y-%m-%d"),
                "processed_through": processed_through.strftime("%Y-%m-%d"),
            }),
            "reason": "sealed_canonical_export", "partitions": {},
            "source_type": "sealed_canonical_export",
        }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if previous_identity.get("contract_fingerprint") != contract_fingerprint:
        # A changed calculator invalidates historical rows as well as today's
        # append. Never merge rows produced under an unknown/older contract.
        args = argparse.Namespace(**vars(args))
        args.force_refresh = True
        args.incremental_start_date = args.start_date
    ...
    if source_identity.get("source_type") == "sealed_canonical_export":
        # A configured SQL URL is not the source while a sealed export is pinned.
        # Revalidate the expected manifest, without hashing the market again.
        _assert_source_unchanged(
            source_identity,
            _partitioned_source_identity(
                args.daily_dir,
                history_start=history_start,
                processed_through=processed_through,
                sql_url=store.config.sql_url,
            ),
            phase="during market read",
        )
    elif revision_identity.get("fingerprint") is not None:
    ...
    elif source_identity.get("source_type") == "sealed_canonical_export" or not store.config.sql_url:
