...
from quant.research.rule_backtest_support import DEFAULT_DAILY_DIR, DEFAULT_OUTPUT_DIR, drop_overlapping_trades
from quant.research.rule_windows import B1_RULE_HISTORY, rule_tail_start
...
def _shared_strategy_prices(raw: pd.DataFrame, price: pd.DataFrame) -> pd.DataFrame:
    """Reuse the exact price scale only if strategy normalization is a no-op.

    Both custom strategies drop invalid/duplicate rows before building prices.
    That changes the adjustment chain, so such inputs retain their original
    independent computation. Removing pre_close marks already-continuous OHLC
    through the price builder's existing no-adjustment contract.
    """
    required = ["date", "open", "high", "low", "close", "volume"]
    if not set(required) <= set(raw.columns) or raw[required].isna().any().any():
        return raw
    dates = pd.to_datetime(raw["date"], errors="coerce")
    if dates.isna().any() or not dates.is_monotonic_increasing or not dates.is_unique:
        return raw
    if "trade_date" in raw.columns:
        trade_dates = pd.to_datetime(raw["trade_date"].astype(str), format="%Y%m%d", errors="coerce")
        if not trade_dates.equals(dates):
            return raw
    # These strategies consume only bars and identity, not the attached factor
    # matrix. Their output here is used only for strategy flags/metrics.
    columns = [*required, "trade_date", "symbol", "ts_code", "name", "pct_chg"]
    return price[[column for column in columns if column in price.columns]].copy()
...
def compute_signal_flags(
    ...
    *,
    ...
    share_continuous_prices: bool = True,
) -> pd.DataFrame:
    ...
    # Keep recursive/anchor strategies on full history. Only finite boolean
    # rules below consume a tail of already-computed factors and prices.
    full_out = out
    price = build_continuous_ohlc(full_out)
    strategy_input = _shared_strategy_prices(full_out, price) if share_continuous_prices else full_out
    full_ma60 = out["ma_60"] if "ma_60" in out.columns else price["close"].rolling(60, min_periods=20).mean()
    finite_rules = [
        spec.name for spec in build_signal_specs()
        if spec.name not in {"signal_vegas_tunnel", "signal_tvb_merged"}
    ]
    offset = rule_tail_start(out, output_start, finite_rules, B1_RULE_HISTORY)
    out = out.iloc[offset:]
    price = price.iloc[offset:]
    close = price["close"]
    open_ = price["open"]
    high = price["high"]
    low = price["low"]
    ...
    try:
        vegas = add_vegas_tunnel_signals(strategy_input, **OPTIMIZED_VEGAS_TUNNEL_PARAMS)
        if offset:
            vegas = vegas.reindex(out.index)
        flags["signal_vegas_tunnel"] = vegas["signal_vegas_tunnel"].fillna(False).astype(bool)
    except Exception:
        vegas = pd.DataFrame(index=out.index)
        flags["signal_vegas_tunnel"] = False

    try:
        triple_volume = add_triple_volume_strategy_pool_signals(strategy_input)
        if offset:
            triple_volume = triple_volume.reindex(out.index)
        flags["signal_tvb_merged"] = triple_volume["signal_tvb_merged"].fillna(False).astype(bool)
        flags["signal_tvb_conservative"] = triple_volume["signal_tvb_conservative"].fillna(False).astype(bool)
        flags["signal_tvb_expanded"] = triple_volume["signal_tvb_expanded"].fillna(False).astype(bool)
    except Exception:
    ...
    if output_start is not None:
        result = result[result["date"] >= pd.Timestamp(output_start)].copy()
    result.attrs["finite_rule_rows"] = len(out)
    result.attrs["full_history_strategy_rows"] = len(full_out)
    return result
    ...
    *,
    ...
    try:
        ...
        if "name" in df.columns:
            names = df["name"].fillna("").astype(str)
            df = df[~names.str.upper().str.contains("ST") & ~names.str.contains("退")].copy()
        signals = compute_signal_flags(df, factors_attached=factors_attached, output_start=output_start)
        signal_cols = [spec.name for spec in build_signal_specs()]
