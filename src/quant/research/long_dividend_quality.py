...
from quant.data.market_snapshot import (
    MarketSnapshotError, current_market_snapshot, pinned_dataset_path, read_pinned_parquet,
)
...
def load_daily_monthly_features(
    ...
    *,
    ...
    if snapshot is not None:
        cache_parts.append(json.dumps(snapshot.manifest["datasets"]["daily"], sort_keys=True))
    if sampling != "monthly":
    ...
    if market.empty and snapshot is None:
        legacy_frames = []
...
def _read_daily_basic_period_end(path: Path) -> pd.DataFrame | None:
    ...
    try:
        frame = read_pinned_parquet(path, columns=list(DAILY_BASIC_FEATURE_COLUMNS))
...
def _daily_basic_source_identity(path: Path) -> dict[str, int | str]:
    snapshot = current_market_snapshot()
    if snapshot is not None:
        for chunk in snapshot.manifest["datasets"]["daily_basic"]["files"]:
            if snapshot.root / chunk["path"] == path.absolute():
                return {"path": chunk["path"], "sha256": chunk["sha256"]}
        raise MarketSnapshotError("Daily-basic cache source is outside the sealed snapshot")
    stat = path.stat()
...
def _daily_basic_source_namespace(source_dir: Path) -> str:
    return "daily_basic:sealed" if current_market_snapshot() is not None else str(source_dir.resolve())
...
def _load_daily_basic_period_cache(
    ...
    try:
        ...
        if (
            ...
            or manifest.get("source_dir") != _daily_basic_source_namespace(source_dir)
        ):
            return pd.DataFrame(columns=DAILY_BASIC_FEATURE_COLUMNS), {}
        ...
        atomic_write_json(
            {
                ...
                "sampling": sampling,
                "source_dir": _daily_basic_source_namespace(source_dir),
...
def load_daily_basic_monthly(
    ...
    *,
    ...
    if sampling not in {"monthly", "weekly"}:
        raise ValueError("sampling must be 'monthly' or 'weekly'")
    source_dir = pinned_dataset_path("daily_basic")
