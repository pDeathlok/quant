...
class MarketDataStore:
    ...
    def _engine(self):
        ...
        return create_engine(self.config.sql_url)

    def _write_sql(self, frame: pd.DataFrame, dataset: str, key: str) -> None:
        table = self._table_name(dataset, key)
        frame.to_sql(table, self._engine(), if_exists="replace", index=False, chunksize=5000, method="multi")

    def _read_sql(self, dataset: str, key: str) -> pd.DataFrame:
        table = self._table_name(dataset, key)
        try:
            return pd.read_sql_table(table, self._engine())
        except Exception:
            return pd.DataFrame()
